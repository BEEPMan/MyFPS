...
public class ObjectPool : MonoBehaviour
{
    ...
    public void CreatePool(string location, string name)
    {
        if (objectPool.TryGetValue(name, out _)) return;
        Pool pool = new Pool();
        GameObject original = Instantiate(Resources.Load($"Prefabs/{location}") as GameObject);
        original.name = name;
    ...
    public GameObject Pop(string location, Vector3 position, Quaternion rotation, Transform parent = null)
    {
        Pool pool;
        string name = GetName(location);
        if(!objectPool.TryGetValue(name, out pool))
        {
            CreatePool(location, name);
